#ifndef GLSL_BXDFS_DIELECTRIC
#define GLSL_BXDFS_DIELECTRIC

#include "fresnel.glsl"
#include "lambert.glsl"
#include "microfacet.glsl"

// https://github1s.com/tunabrain/tungsten/blob/master/src/core/bsdfs/RoughDielectricBsdf.cpp
// https://github.com/mitsuba-renderer/mitsuba/blob/master/src/bsdfs/roughdielectric.cpp

vec3 /*brdf*/ dielectric_eval(const vec3 wi, const vec3 wo, vec3 n, const vec3 albedo, const float roughness, 
	...
	float cosThetaT;
	const float fi = F_Dielectric(eta_i, eta_t, wiDotM, cosThetaT);
	const float ggx = ggx_eval(wi, wo, n, m, roughness);
	vec3 result = vec3(1.0f);//albedo;
	if (reflection) {
		result *= fi * vec3(abs(woDotN) * ggx * BSDF_GGX(wiDotN, woDotN));
	} else {
		result *= (1.0f - fi) * vec3(abs(woDotN) * ggx * BSDF_GGX(wiDotM, woDotM, wiDotN, woDotN, eta_i, eta_t));
...
float /*pdf*/ dielectric_pdf(const vec3 wi, const vec3 wo, vec3 n, const float roughness, 
							 float eta_int, float eta_ext) {
	const bool rZero = (roughness == 0.0f);
	...
	const float woDotM = dot(wo, m);
	// make sure we are in the upper hemisphere
	//if (wiDotN <= 0.0f) return 0; //!!danger!!
	// make sure wi/wo are not parallel
	...
	float cosThetaT;
	const float fi = F_Dielectric(eta_i, eta_t, wiDotM, cosThetaT);
	float pdf = ggx_pdf(n, m, wi, roughness);

	if (reflection) pdf *= D_PDF_M_TO_WO(woDotM);
	else pdf *= D_PDF_M_TO_WO(wiDotM, woDotM, eta_i, eta_t);
...
bool dielectric_sample(const vec3 wi, vec3 n, const vec3 albedo, const float roughness, float eta_int, float eta_ext, 
	...
	if (reflection) /* specular */ {
		...
		wo = refractionDir(wi, m, eta_i, eta_t, cosThetaT);
		// total internal reflection
		if (all(equal(wo, vec3(0)))) return false;
		if (eta_i == eta_t) wo = -wi;
	}
	...
	if (reflection) /* specular */ {
		...
		pdf *= (1.0f - fi);
		// account for solid angle compression that occurs when crossing the interface
		const float factor = cosThetaT < 0.0f ? eta_i / eta_t : eta_t / eta_i;
