...
{
	// produce a string in the form of "Scene (.gltf/.glb)\0*.gltf;*.glb\0";
	int pointer = 0;
	...
	//ofn.hwndOwner = Common::getInstance().getMainWindow()->window()->getWindowHandle();
...
std::filesystem::path sys::saveFileDialog(const std::string& aTitle, const std::string& aExtension)
{
	...
	ofn.lpstrTitle = aTitle.c_str();
	ofn.lStructSize = sizeof(ofn);
	//ofn.hwndOwner = (HWND)Common::getInstance().getMainWindow()->window()->getWindowHandle();
	ofn.lpstrFile = szFile;
	ofn.nMaxFile = sizeof(szFile);
	ofn.lpstrInitialDir = nullptr;
