...
void EventSystem::queueEvent(const EventType aType, const Input aInput, const int aValue, const float aX, const float aY, const std::string& aString) {
	EventSystem& es = getInstance();
	// make sure only one thread at a time can queue/process events
	...
	if (es.mEventHead - es.mEventTail >= MAX_QUED_EVENTS) {
		//spdlog::warn("Event Queue Overflow");
		// discarding
		es.mEventTail++;
	}
	es.mEventHead++;
...
void EventSystem::processEvent(const Event& aEvent)
{
	...
	if (aEvent.isKeyEvent()) {
		...
		case Input::A_TAKE_SCREENSHOT:
			common.takeScreenshot(aEvent.getMessage(), !aEvent.getValue());
			break;
		case Input::A_EXIT:
			common.shutdown();
			break;
