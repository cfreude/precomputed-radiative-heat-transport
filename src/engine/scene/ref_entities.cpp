...
void Ref_s::updateSceneGraphNodesFromModelMatrix(bool aFlipY) const
{
	if (transforms.empty()) return;
	glm::mat4 sg_model_matrix = glm::mat4(1.0f);
	for (TRS* trs : transforms) {
...
glm::vec3 RefCamera_s::getPosition()
{
	return model_matrix[3];
}

glm::vec3 RefCamera_s::getDirection()
{
	return -model_matrix[2];
}

RefCameraPrivate_s::RefCameraPrivate_s() : RefCamera_s(), default_camera(false),
axis{ {1,0,0}, {0,1,0}, {0,0,1} },
angles({0,0,0}), position({0, 0, 40})
{}

// fps
bool RefCameraPrivate_s::updateAngles(const float aNewMx, const float aNewMy)
{
	...
	// keep editor cam consistent with fps cam
	editor_data.pivot = position - axis[2] * editor_data.zoom;
	return true;
}

bool RefCameraPrivate_s::updatePosition(const bool aFront, const bool aBack, const bool aLeft, const bool aRight, const bool aUp, const bool aDown) {
	if (!(aFront || aBack || aLeft || aRight || aUp || aDown)) return false;
	const float forward = static_cast<float>(aBack) * fps_data.movement_speed - static_cast<float>(aFront) * fps_data.movement_speed;	// we move forward in -z direction
	...
	float speed = glm::length(fps_data.current_velocity);
...
bool RefCameraPrivate_s::updateSpeed(const float aNewMz)
{
...
// editor
bool RefCameraPrivate_s::drag(const glm::vec2 aDelta)
{
...
bool RefCameraPrivate_s::updateCenter(const glm::vec2 aDelta)
{
	if (glm::all(glm::equal(aDelta, glm::vec2(0)))) return false;
...
bool RefCameraPrivate_s::updateZoom(const float aNewMz)
{
	if (aNewMz == 0.0f) return false;
...
void RefCameraPrivate_s::setModelMatrix(glm::mat4& aMatrix, const bool aFlipY)
{
	axis[0] = aMatrix[0];
	axis[1] = aMatrix[1];
	axis[2] = aMatrix[2];
...
void RefCameraPrivate_s::updateMatrix(const bool aFlipY)
{
...
Frustum::Frustum(const RefCamera_s* c)
{
...
bool Frustum::checkAABBInside(glm::vec3 min, glm::vec3 max) const
{
	for (size_t i = 0; i < static_cast<size_t>(Plane::COUNT); i++)
	{
		glm::vec3 p = min;
		...
		//glm::vec3 n = max;
		//if (plane[i].n.x >= 0) n.x = min.x;
		//if (plane[i].n.y >= 0) n.y = min.y;
		//if (plane[i].n.z >= 0) n.z = min.z;
