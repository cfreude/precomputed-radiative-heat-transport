...
void Exporter::save_image_png_8_bit(std::string const& aName, const int aWidth, const int aHeight, const int aChannels, const uint8_t* aPixels)
{
    stbi_write_png(aName.c_str(), aWidth, aHeight, aChannels, aPixels, aWidth * aChannels);
}

void Exporter::save_image_exr(std::string const& aName, const int aWidth, const int aHeight, const float* aPixels, const uint32_t aStride, const std::vector<uint8_t>& aOut)
{
    EXRHeader header;
