...
namespace {
	...
	// load the custom properties set into the extra field of the gltf file
	...
	int exportImage(const std::filesystem::path& aOutDir, tinygltf::Model& aGltfModel, tamashii::Image* aImage)
	{
		...
		// if outFilepath already exists, look for new name
		uint32_t appendix = 0;
	...
	void exportImages(const std::filesystem::path& aOutDir, tinygltf::Model& aGltfModel, const SceneInfo_s& aSceneInfo)
	{
	...
	void exportTexture(tinygltf::Model& aGltfModel, tamashii::Texture* aTexture) {
		...
		sam.wrapR = textureWrapToTinygltfTextureWrapType(aTexture->sampler.wrapW);
		int samplerIndex = -1;
	...
	uint64_t calcBufferSize(const SceneInfo_s& aSceneInfo)
	{
		uint64_t size = 0;
		for(const Model* model : aSceneInfo.mModels)
		{
			for(const Mesh* mesh : model->getMeshList())
			{
				if (mesh->hasIndices()) {
				...
				}
				{
					...
					for (const auto& [id, data] : mesh->getCustomDataMapConstRef())
					{
		...
		if (!aSceneInfo.mSceneGraphs.empty()) {
			...
			aSceneInfo.mSceneGraphs[0]->applyFunction([&size](Node* aNode)
			{
				const TRS* trs = aNode->getTRS();
				if (trs->hasTranslationAnimation())
				{
					const uint64_t ts = trs->translation_steps.size();
					const uint64_t tts = trs->translation_time_steps.size();
					size += ts ? rountUpToMultipleOf(ts * sizeof(trs->translation_steps[0]), ROUND_UP_CONST) : 0;
					size += tts ? rountUpToMultipleOf(tts * sizeof(trs->translation_time_steps[0]), ROUND_UP_CONST) : 0;
				}
				if (trs->hasRotationAnimation())
				{
					const uint64_t rs = trs->rotation_steps.size();
					const uint64_t rts = trs->rotation_time_steps.size();
					size += rs ? rountUpToMultipleOf(rs * sizeof(trs->rotation_steps[0]), ROUND_UP_CONST) : 0;
					size += rts ? rountUpToMultipleOf(rts * sizeof(trs->rotation_time_steps[0]), ROUND_UP_CONST) : 0;
				}
				if (trs->hasScaleAnimation())
				{
					const uint64_t ss = trs->scale_steps.size();
					const uint64_t sts = trs->scale_time_steps.size();
					size += ss ? rountUpToMultipleOf(ss * sizeof(trs->scale_steps[0]), ROUND_UP_CONST) : 0;
					size += sts ? rountUpToMultipleOf(sts * sizeof(trs->scale_time_steps[0]), ROUND_UP_CONST) : 0;
				}
			});
		}
		return size;
	...
	tinygltf::Primitive exportMesh(tinygltf::Model& aGltfModel, gltfBuffer& aBuffer, tamashii::Mesh* aMesh)
	{
		...
		}
		{
			...
			for(const auto& [id, data] : aMesh->getCustomDataMapRef())
			{
    ...
    void exportModel(tinygltf::Model& aGltfModel, gltfBuffer& aBuffer, tamashii::Model* aModel)
    {
		...
		for(tamashii::Mesh* mesh : aModel->getMeshList())
		{
			tmesh.primitives.push_back(exportMesh(aGltfModel, aBuffer, mesh));
		}
	...
	void exportLight(tinygltf::Model& aGltfModel, tamashii::Light* aLight)
	{
		...
		light.intensity = static_cast<double>(aLight->getIntensity());
		...
		case Light::Type::POINT:
			light.type = "point";
			light.range = static_cast<double>(((PointLight*)aLight)->getRange());
			break;
		case Light::Type::SPOT:
			light.type = "spot";
			light.range = static_cast<double>(((SpotLight*)aLight)->getRange());
			light.spot.innerConeAngle = static_cast<double>(((SpotLight*)aLight)->getInnerConeAngle());
			light.spot.outerConeAngle = static_cast<double>(((SpotLight*)aLight)->getOuterConeAngle());
			break;
		case Light::Type::SURFACE:
		{
			tamashii::SurfaceLight* sl = ((SurfaceLight*)aLight);
		...
		case Light::Type::IES:
		{
			light.type = "spot";
			light.range = static_cast<double>(((SpotLight*)aLight)->getRange());
			aLight->addCustomProperty("ies", Value(std::string(((IESLight*)aLight)->getCandelaTexture()->image->getName())));
			break;
		}
		}
	...
	void exportAnimation(tinygltf::Model& aGltfModel, gltfBuffer& aBuffer, const tamashii::TRS* aTrs, const int aNodeIndex)
	{
		...
		if (aTrs->hasTranslationAnimation())
		{
			...
			const int inPosAccessor = writeTimeStepsToBuffer(aGltfModel, aBuffer, aTrs->translation_time_steps);

			const int outCount = static_cast<int>(aTrs->translation_steps.size());
			const uint64_t outSizeInBytes = outCount * sizeof(aTrs->translation_steps[0]);
			...
			for (int i = 0; i < outCount; i++) outData[i] = aTrs->translation_steps[i];
			...
			sampler.interpolation = interpolationToString(aTrs->trans_anim_inter);
		...
		if (aTrs->hasRotationAnimation()) 
		{
			...
			const int inPosAccessor = writeTimeStepsToBuffer(aGltfModel, aBuffer, aTrs->rotation_time_steps);

			const int outCount = static_cast<int>(aTrs->rotation_steps.size());
			const uint64_t outSizeInBytes = outCount * sizeof(aTrs->rotation_steps[0]);
			const auto outData = reinterpret_cast<glm::vec4*>(aBuffer.mPointer + aBuffer.mData.data());
			const int outPosAccessor = getAccessor(aGltfModel, aBuffer, outSizeInBytes, outCount);
			for (int i = 0; i < outCount; i++) outData[i] = aTrs->rotation_steps[i];
			aGltfModel.accessors[outPosAccessor].componentType = TINYGLTF_COMPONENT_TYPE_FLOAT;
			aGltfModel.accessors[outPosAccessor].type = TINYGLTF_TYPE_VEC4;

			tinygltf::AnimationSampler sampler;
			sampler.interpolation = interpolationToString(aTrs->rot_anim_inter);
			sampler.input = inPosAccessor;
			sampler.output = outPosAccessor;
			animation.samplers.push_back(sampler);
		}
		if (aTrs->hasScaleAnimation())
		{
			...
			const int inPosAccessor = writeTimeStepsToBuffer(aGltfModel, aBuffer, aTrs->scale_time_steps);

			const int outCount = static_cast<int>(aTrs->scale_steps.size());
			const uint64_t outSizeInBytes = outCount * sizeof(aTrs->scale_steps[0]);
			const auto outData = reinterpret_cast<glm::vec3*>(aBuffer.mPointer + aBuffer.mData.data());
			const int outPosAccessor = getAccessor(aGltfModel, aBuffer, outSizeInBytes, outCount);
			for (int i = 0; i < outCount; i++) outData[i] = aTrs->scale_steps[i];
			aGltfModel.accessors[outPosAccessor].componentType = TINYGLTF_COMPONENT_TYPE_FLOAT;
			aGltfModel.accessors[outPosAccessor].type = TINYGLTF_TYPE_VEC3;

			tinygltf::AnimationSampler sampler;
			sampler.interpolation = interpolationToString(aTrs->scale_anim_inter);
			sampler.input = inPosAccessor;
			sampler.output = outPosAccessor;
			animation.samplers.push_back(sampler);
		}
	...
	int exportNode(tinygltf::Model& aGltfModel, gltfBuffer& aBuffer, tamashii::Node* aNode)
	{
		...
		if (aNode->hasModel())
		{
			tamashii::Model* model = aNode->getModel();
			if (modelToIndexDirectory.find(model) != modelToIndexDirectory.end()) {
				tnode.mesh = modelToIndexDirectory[model];
			}
		}
		if (aNode->hasCamera()) {
			tamashii::Camera* cam = aNode->getCamera();
			if (cameraToIndexDirectory.find(cam) != cameraToIndexDirectory.end()) {
				tnode.camera = cameraToIndexDirectory[cam];
			}
		}
		if (aNode->hasLight()) {
			tamashii::Light* light = aNode->getLight();
		...
		tnode.children.reserve(aNode->numNodes());
		for(tamashii::Node* n : *aNode)
		{
	...
	void exportScene(tinygltf::Model& aGltfModel, gltfBuffer& aBuffer, tamashii::SceneGraph* aSceneGraph)
	{
		tinygltf::Scene gltfScene;
		gltfScene.name = aSceneGraph->getName();
		gltfScene.nodes.reserve(aSceneGraph->getRootNodes().size());
		for (tamashii::Node* n : *aSceneGraph)
		{
...
bool Exporter::save_scene_gltf(const std::string& aOutputFile, const SceneExportSettings aSettings, const SceneInfo_s& aSceneInfo)
{
	...
	for (const tamashii::Model* mod : aSceneInfo.mModels) for(tamashii::Mesh* me : mod->getMeshList()) accessorCount += 7;
	if (!aSceneInfo.mSceneGraphs.empty()) {
		uint32_t nodeCount = 0;
		aSceneInfo.mSceneGraphs[0]->applyFunction([&nodeCount, &accessorCount, &animationCount](Node* aNode)
		{
			nodeCount++;
			TRS* trs = aNode->getTRS();
			if (trs) {
	...
	if (!aSettings.mExcludeModels) {
		...
		for (tamashii::Model* mod : aSceneInfo.mModels) exportModel(model, dataBuffer, mod);
	}
	if (!aSettings.mExcludeCameras) for (tamashii::Camera* cam : aSceneInfo.mCameras) exportCamera(model, cam);
	if (!aSettings.mExcludeLights) for (tamashii::Light* light : aSceneInfo.mLights) exportLight(model, light);
...
namespace {
	...
	int textureWrapToTinygltfTextureWrapType(const Sampler::Wrap aTextureWrap) {
		switch (aTextureWrap)
		{
			case Sampler::Wrap::REPEAT: return TINYGLTF_TEXTURE_WRAP_REPEAT;	// REPEAT
			case Sampler::Wrap::CLAMP_TO_EDGE: return TINYGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE;	// CLAMP_TO_EDGE
			case Sampler::Wrap::MIRRORED_REPEAT: return TINYGLTF_TEXTURE_WRAP_MIRRORED_REPEAT;	// MIRRORED_REPEAT
	...
	const char* interpolationToString(const tamashii::TRS::Interpolation aInterpolation)
	{
		...
		case TRS::Interpolation::NONE: {
			...
			case TRS::Interpolation::CUBICSPLINE: return "CUBICSPLINE";
		}
		return "";
	}
}
