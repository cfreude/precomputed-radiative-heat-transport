...
public:
	...
	struct QueueFamily_s {
		...
		std::map<std::string, bool>					presentation_support;
	...
	bool											checkQueueFamily(int aFamilyIndex, uint32_t aQueueFlags, const std::vector<std::string>& aPresentationSupport = {}) const;
													// find a family best suited for the requirements
	int												getQueueFamilyIndex(uint32_t aQueueFlags, const std::vector<std::string>& aPresentationSupport = {}) const;
	...
	const VkSurfaceCapabilitiesKHR&					getSurfaceCapabilities(const std::string& aSurfaceId);
	const std::vector<VkSurfaceFormatKHR>&			getSurfaceFormats(const std::string& aSurfaceId);
	const std::vector<VkPresentModeKHR>&			getSurfacePresentModes(const std::string& aSurfaceId);
#endif

	/// <summary>
	///	Create a Logical Device from a Physical Device
	/// </summary>
	/// <param name="aExtensions">Extensions to activate</param>
	/// <param name="aQueues">Queues to activate, pair of queue family index and priorities (length describes the amount of queues)</param>
	/// <param name="pNext">Additional data</param>
...
private:
	...
	std::map<std::string, SurfaceInfos_s>			mSurfaces;
#endif
