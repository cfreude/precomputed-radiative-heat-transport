...
// used for single time command recording/submission
// commandPool is the pool where a command buffer is allocated
// uint32_t is the queue index of the queue family to which commandPool belongs
...
public:
	...
	// If a swapchain is used, the first queue that supports everything is used. If presentation and rendering are two queues, then
	//	the first queue that supports presentation and the first queue that supports rendering are used
	Swapchain*										createSwapchain(const std::string& aSurfaceId);
	std::deque<Swapchain*>&							getSwapchains();
	void											destroySwapchain(Swapchain*& aSwapchain);
...
private:
	...
	std::deque<Swapchain*>							mSwapchains;
