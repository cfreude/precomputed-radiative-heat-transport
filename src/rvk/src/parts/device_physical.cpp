...
bool PhysicalDevice::checkQueueFamily(const int aFamilyIndex, const uint32_t aQueueFlags,
	const std::vector<std::string>& aPresentationSupport) const
{
...
int PhysicalDevice::getQueueFamilyIndex(const uint32_t aQueueFlags, const std::vector<std::string>& aPresentationSupport) const
{
	const bool reqGraphics = aQueueFlags & PhysicalDevice::Queue::GRAPHICS;
	const bool reqCompute = aQueueFlags & PhysicalDevice::Queue::COMPUTE;
	const bool reqTransfer = aQueueFlags & PhysicalDevice::Queue::TRANSFER;
	...
	for (uint32_t i = 0; i < mQueueFamilies.size(); i++) {
		// check for requirements and score the queue family
		...
		if(compatible)
		{
			// nothing found yet -> assign it
			if (bestFamily == -1) {
				bestFamily = static_cast<int>(i);
				bestScore = score;
			}
			// else, use only if it is closer to the minimum requirements
...
const VkSurfaceCapabilitiesKHR& PhysicalDevice::getSurfaceCapabilities(const std::string& aSurfaceId)
{
	SurfaceInfos_s& si = mSurfaces[aSurfaceId];
...
const std::vector<VkSurfaceFormatKHR>& PhysicalDevice::getSurfaceFormats(const std::string& aSurfaceId)
{
	return mSurfaces[aSurfaceId].surfaceFormats;
}

const std::vector<VkPresentModeKHR>& PhysicalDevice::getSurfacePresentModes(const std::string& aSurfaceId)
{
	return mSurfaces[aSurfaceId].surfacePresentModes;
}
#endif
...
PhysicalDevice::PhysicalDevice(Instance* aInstance, VkPhysicalDevice aPhysicalDevice, uint32_t aPhysicalDeviceIndex, VkPhysicalDeviceGroupProperties aGroupProperties) : mInstance(aInstance), mPhysicalDeviceIndex(aPhysicalDeviceIndex), mPhysicalDevice(aPhysicalDevice), mPhysicalDeviceGroupProperties({}), mProperties({}), mFeatures({}), mMemoryProperties({}),
                                                     mFeatures2({}), mAccelerationStructureFeatures({}), mProperties2({}),
                                                     mMemoryProperties2({}), mRayTracingPipelineProperties({})
{
	...
	//	/*aInstance->vk.GetPhysicalDeviceQueueFamilyProperties2(mPhysicalDevice, &count, VK_NULL_HANDLE);
	//	std::vector<VkQueueFamilyProperties2> queueFamilyProperties2(count);
	//	aInstance->vk.GetPhysicalDeviceQueueFamilyProperties2(mPhysicalDevice, &count, queueFamilyProperties2.data());

	//	VkPhysicalDeviceMemoryProperties2 memoryProperties2;
	//	memoryProperties2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MEMORY_PROPERTIES_2;
	//	aInstance->vk.GetPhysicalDeviceMemoryProperties2(mPhysicalDevice, &memoryProperties2);*/
	}
