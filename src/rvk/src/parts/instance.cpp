...
{
#ifdef WIN32
	return GetProcAddress(static_cast<HMODULE>(aLib), aName.c_str());
#elif defined( __linux__ ) || defined(__APPLE__)
...
bool InstanceManager::areLayersAvailable(const std::vector<const char*>& aLayers) const
{
	...
	for (auto layer : aLayers)
	{
		if (!isLayerAvailable(layer)) {
			Logger::warning("requested instance layer is not available: " + std::string(layer));
			available = false;
		}
	}
	return available;
...
#define VK_INSTANCE_LEVEL_FUNCTION(fn) vk.fn = (PFN_vk##fn) vkGetInstanceProcAddr( aInstance , "vk"#fn );// if(!fn) Logger::warn("could not load instance function: "#fn);
#include <rvk/vk_function_list.inl>
...
VkSurfaceKHR Instance::registerSurface(const std::string& aSurfaceId, void* const p1, void* const p2)
{
	VkSurfaceKHR surface = VK_NULL_HANDLE;
	if (aSurfaceId.empty() || !isExtensionActive(VK_KHR_SURFACE_EXTENSION_NAME)) return surface;
#ifdef WIN32
	...
	desc.window = *((xcb_window_t*)p1);
	desc.connection = (xcb_connection_t*)p2;
    VK_CHECK(vk.CreateXcbSurfaceKHR(mInstance, &desc, nullptr, &surface), "failed to create xcb Surface!");
#endif
#endif
	if (surface) {
		Surface s = { surface, std::pair(p1, p2) };
		mSurfaces.insert(std::pair(aSurfaceId, s));
		...
		for (PhysicalDevice* d : mPhysicalDevices) {
			...
			d->mSurfaces.insert(std::pair(aSurfaceId, infos));
			...
			for (PhysicalDevice::QueueFamily_s& f : d->mQueueFamilies) {
				...
				f.presentation_support.insert(std::pair<std::string, bool>(aSurfaceId, support));
			}
		}
	}
	return surface;
...
Instance::Surface Instance::getSurface(const std::string& aSurfaceId)
{
	if (aSurfaceId.empty() || (mSurfaces.find(aSurfaceId) == mSurfaces.end())) return {};
	else return mSurfaces[aSurfaceId];
}
