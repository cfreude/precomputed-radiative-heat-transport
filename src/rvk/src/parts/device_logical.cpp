...
#define VK_DEVICE_LEVEL_FUNCTION(fn) vk.fn = (PFN_vk##fn) instance->vk.GetDeviceProcAddr( mLogicalDevice , "vk"#fn );// if(!this->fn) Logger::warn("could not load instance function: "#fn);
#define VK_DEVICE_LEVEL_CMD_FUNCTION(fn) vkCmd.fn = (PFN_vkCmd##fn) instance->vk.GetDeviceProcAddr( mLogicalDevice , "vkCmd"#fn );// if(!this->fn) Logger::warn("could not load instance function: "#fn);
...
LogicalDevice::~LogicalDevice()
{
	...
	for (const Swapchain* s : mSwapchains) delete s;
	mSwapchains.clear();
...
int LogicalDevice::getQueueFamilyIndex(const uint32_t aQueueFlags, const std::vector<std::string>& aPresentationSupport) const
{
...
Swapchain* LogicalDevice::createSwapchain(const std::string& aSurfaceId)
{
	mSwapchains.push_back(new Swapchain(this, aSurfaceId));
	return mSwapchains.back();
}

std::deque<Swapchain*>& LogicalDevice::getSwapchains()
{
	return mSwapchains;
}

void LogicalDevice::destroySwapchain(Swapchain*& aSwapchain)
{
	const auto it = std::find(mSwapchains.begin(), mSwapchains.end(), aSwapchain);
