...
Swapchain::Swapchain(LogicalDevice* aDevice, const std::string& aSurfaceId) : mDevice(aDevice), mSurfaceId(aSurfaceId),
mExtent({0, 0}), mSurfaceFormat({}), mDepthFormat(VK_FORMAT_UNDEFINED),
mImageCount(0), mSwapchain(VK_NULL_HANDLE), mRpClear(nullptr), mRpKeep(nullptr), mPreviousIndex(-1), mCurrentIndex(-1)
{
	mSurface = aDevice->getPhysicalDevice()->getInstance()->getSurface(aSurfaceId).mHandle;
...
{
	return mDevice->getPhysicalDevice()->getSurfaceCapabilities(mSurfaceId);
}

const std::vector<VkSurfaceFormatKHR>& Swapchain::getSurfaceFormats() const
{
	return mDevice->getPhysicalDevice()->getSurfaceFormats(mSurfaceId);
}

const std::vector<VkPresentModeKHR>& Swapchain::getSurfacePresentModes() const
{
	return mDevice->getPhysicalDevice()->getSurfacePresentModes(mSurfaceId);
}
...
{
	for(const VkPresentModeKHR e : mDevice->getPhysicalDevice()->getSurfacePresentModes(mSurfaceId))
	{
...
{
	for (const auto [format, colorSpace] : mDevice->getPhysicalDevice()->getSurfaceFormats(mSurfaceId))
	{
...
{
	const uint32_t min = getSurfaceCapabilities().minImageCount;
	const uint32_t max = getSurfaceCapabilities().maxImageCount;
...
void Swapchain::destroy()
{
	...
	if (mRpClear) {
		mDevice->vk.DestroyRenderPass(mDevice->getHandle(), mRpClear->mRenderPass, nullptr);
		delete mRpClear;
		mRpClear = nullptr;
	}
	if (mRpKeep) {
		mDevice->vk.DestroyRenderPass(mDevice->getHandle(), mRpKeep->mRenderPass, nullptr);
		delete mRpKeep;
		mRpKeep = nullptr;
	}
	destroyImages();
...
std::string Swapchain::getSurfaceId()
{
	return mSurfaceId;
}
...
void Swapchain::createRenderpasses()
{
	...
	//mRpClear->addDependency(0, 0, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, VK_DEPENDENCY_BY_REGION_BIT);
	mRpClear->finish();
...
void Swapchain::createSwapChain()
{
	...
	//queue_family_indices.push_back(present_queue_family);
	//if (present_queue_family != render_queue_family) {
	//	queue_family_indices.push_back(render_queue_family);
	//	createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
	//	createInfo.queueFamilyIndexCount = /* 2 */ static_cast<uint32_t>(queue_family_indices.size());
