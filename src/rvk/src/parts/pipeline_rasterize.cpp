...
void rvk::RPipeline::createPipeline(const bool aFront) {
    // if this is the first build save the state with which the pipeline was build
    ...
    rs.cullMode = mLocalRenderState.cullMode;//VK_CULL_MODE_BACK_BIT; // we want the back face as well
    ...
    pipelineInfo.renderPass = mLocalRenderState.renderpass;
    VK_CHECK(mDevice->vk.CreateGraphicsPipelines(mDevice->getHandle(), mCache, 1, &pipelineInfo, NULL, aFront ? &mHandleFront : &mHandleBack), " failed to create Pipeline!");
}
