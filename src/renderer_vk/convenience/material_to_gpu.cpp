...
void MaterialDataVulkan::loadScene(rvk::SingleTimeCommand* aStc, const scene_s aScene, TextureDataVulkan* aTextureDataVulkan)
{
	unloadScene();

	if(mAutoResize && aScene.materials.size() > mMaxMaterialCount)
	{
		...
		destroy();
		prepare(materialBufferUsageFlags, static_cast<uint32_t>(aScene.materials.size()));
		mBufferResized = true;
	}

	if (aScene.materials.empty()) return;
	for (Material *material : aScene.materials) {
...
void MaterialDataVulkan::update(rvk::SingleTimeCommand* aStc, const scene_s aScene, TextureDataVulkan* aTextureDataVulkan)
{
	unloadScene();
	loadScene(aStc, aScene, aTextureDataVulkan);
}
